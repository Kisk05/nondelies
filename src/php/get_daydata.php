...
try{
    ...
    if ($setDay) {
        ...
        if (count($parts) === 3) {
            ...
            $oneday = "{$parts[0]}-{$paddedMonth}-{$paddedDay}";
        }
    }

    if (is_null($sup_ids) || is_null($yearday) || is_null($day) || is_null($oneday) || is_null($dayofweek)) {
    ...
    $where_condition = "e.event_dayofweek = ? OR e.event_yearday = ? OR e.event_monthday = ? OR e.event_oneday = ?";
